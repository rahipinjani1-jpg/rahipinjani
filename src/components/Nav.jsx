    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-100 flex items-center justify-between px-8 md:px-15 h-17 bg-warm-white/92 backdrop-blur-md border-b border-rule">
        ...
        >
          ...
          <span className="hidden md:inline">Rahi</span>
        </a>
        ...
        <div className="hidden lg:flex gap-10 items-center">
          {links.map(({ id, label }) => {
            const isActive =
              currentPage === id || (id === "projects" && isProjectDetail);
            return (
              <a
                key={id}
                onClick={() => onNav(id)}
      ...
      <div
        className={`fixed inset-0 top-17 z-90 bg-warm-white flex flex-col lg:hidden transition-opacity duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        {/* Nav links */}
        ...
        <div className="px-10 pb-12 pt-8">
          <p className="text-[9px] tracking-[0.26em] uppercase text-muted mb-6">
            Get in touch
          </p>
